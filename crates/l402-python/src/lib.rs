...
//! Python bindings for the L402sdk L402 client SDK.
//!
...
use l402_core::budget::Budget as RustBudget;
use l402_core::cache::InMemoryTokenStore;
use l402_core::receipt::Receipt as RustReceipt;
use l402_core::{L402Client as RustClient, L402ClientConfig};
use l402_lnd::{LndGrpcBackend, LndRestBackend};
...
/// Example::
///
///     from l402 import LndRestBackend
///
...
/// Example::
///
///     from l402 import LndGrpcBackend
///
...
/// Example::
///
///     from l402 import ClnRestBackend
///
...
/// Example::
///
///     from l402 import ClnGrpcBackend
///
...
/// Example::
///
///     from l402 import SwissKnifeBackend
///
...
fn build_client<B: LnBackend + 'static>(
    ...
    let config = L402ClientConfig {
        max_fee_sats,
        max_retries: 1,
        user_agent: format!("l402-python/{}", env!("CARGO_PKG_VERSION")),
    };
...
fn map_client_error(err: &l402_proto::ClientError) -> PyErr {
    match err {
        l402_proto::ClientError::BudgetExceeded { .. } => {
            PyValueError::new_err(format!("BudgetExceeded: {err}"))
        }
        l402_proto::ClientError::PaymentFailed { .. } => {
            PyRuntimeError::new_err(format!("PaymentFailed: {err}"))
        }
        l402_proto::ClientError::MissingChallenge => {
...
#[pymodule]
fn _l402(m: &Bound<'_, PyModule>) -> PyResult<()> {
